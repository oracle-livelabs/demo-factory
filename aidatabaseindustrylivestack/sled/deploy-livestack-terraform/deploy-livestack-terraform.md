...
2. Select the following link to open the stack-creation page with the State and Local Terraform package already selected:

    [Deploy the State and Local LiveStack with OCI Resource Manager](https://cloud.oracle.com/resourcemanager/stacks/create?zipUrl=https://github.com/oracle-livelabs/livestack/raw/refs/heads/main/aidatabaseindustrylivestack/terraform-ready-livestack/sled/state-local-livestack-terraform.zip)

3. On the **Create stack** page, confirm that the **Package URL** identifies `state-local-livestack-terraform.zip`. No local download or ZIP upload is required.
